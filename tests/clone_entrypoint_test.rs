...
fn entrypoint_runs_pg_basebackup_from_source_as_pgreplica() {
    ...
    let script = generate_clone_entrypoint("pgforge_billing", 5432);
...
fn entrypoint_execs_official_postgres_entrypoint_with_bindmount_config_flags() {
    // Without `-c config_file=...` and `-c hba_file=...`, postgres reads
    // only $PGDATA-internal config files generated by initdb — our
    // bind-mounted postgresql.conf and pg_hba.conf would be silently
    // ignored. Verify both flags ride along on the final exec.
    let script = generate_clone_entrypoint("pgforge_billing", 5432);
...
fn uses_provided_port() {
    let s = generate_clone_entrypoint("src", 6543);
