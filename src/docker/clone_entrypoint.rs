...
pub fn generate_clone_entrypoint(source_container: &str, source_port: u16) -> String {
    format!(
        r#"#!/bin/sh
...
if [ ! -f "$MARKER" ]; then
    ...
    su - postgres -c "PGPASSFILE=$PGPASSFILE_RUNTIME pg_basebackup -h {source} -p {port} -U pgreplica -D $PGDATA -X stream -P --wal-method=stream --no-password"
    touch "$MARKER"
